    ...
    <div class="flex h-full w-full flex-1 flex-col gap-6">
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm p-4 border border-gray-100 dark:border-gray-700">
            <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                <div>
                    <h1 class="text-2xl font-bold text-gray-800 dark:text-white">Gestión de Alumnos</h1>
                    <p class="text-gray-500 dark:text-gray-400">Listado completo de estudiantes registrados</p>
                </div>
                
                <div class="flex flex-col sm:flex-row gap-3">
                    <!-- Search Form -->
                    <form action="{{ route('administracion.alumnos.buscar') }}" method="GET" class="flex-1 min-w-[250px]">
                        <div class="relative">
                    ...
                    <!-- Add Button -->
                    <a href="{{ route('administracion.alumnos.create')}}" 
                ...
                <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                    ...
                    <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                        ...
                        <tr class="bg-white hover:bg-gray-50 dark:hover:bg-gray-700 dark:bg-gray-800">
                            <td class="px-6 py-4 font-medium text-gray-900 dark:text-white">{{ $alumno->id }}</td>
                            <td class="px-6 py-4">
                                <div class="font-medium text-gray-900 dark:text-white">{{ $alumno->AP }} {{ $alumno->AM }}</div>
                                <div class="text-sm text-gray-500">{{ $alumno->Nombre }}</div>
                            </td>
                            <td class="px-6 py-4 font-mono">
                                <a href="{{ route('administracion.alumnos.show', $alumno) }}" class="text-blue-600 hover:underline dark:text-blue-400">
                                    {{ $alumno->CURP }}
                                </a>
                            </td>
                            ...
                            <td class="px-6 py-4">
                                <div class="flex justify-end space-x-2">
                                    <a href="{{ route('administracion.alumnos.edit', $alumno) }}" 
                        ...
                        <tr>
                            <td colspan="5" class="px-6 py-4 text-center text-gray-500 dark:text-gray-400">
                                @if(request()->has('alumno'))
                                    No se encontraron alumnos con "{{ request('alumno') }}"
                                @else
